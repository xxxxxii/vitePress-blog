...
import { pathToFile } from '../utils/utils'
    ...
    <div ref="el" role="button" :aria-owns="results?.length ? 'localsearch-list' : undefined" aria-expanded="true"
      ...
      <div class="shell">
        <form class="search-bar" @pointerup="onSearchBarClick($event)" @submit.prevent="">
          ...
          <input ref="searchInput" v-model="filterText" :placeholder="buttonText" id="localsearch-input"
            aria-labelledby="localsearch-label" class="search-input" />
          <div class="search-actions">
            <button v-if="!disableDetailedView" class="toggle-layout-button" type="button"
              :class="{ 'detailed-list': showDetailedList }" :title="$t('modal.displayDetails')" @click="
      ...
      ">
            ...
            <button class="clear-button" type="reset" :disabled="disableReset" :title="$t('modal.resetButtonTitle')"
              @click="resetSearch">
              <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
                <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <ul ref="resultsEl" :id="results?.length ? 'localsearch-list' : undefined"
          ...
          <li v-for="(p, index) in results" :key="p.id" role="option"
            :aria-selected="selectedIndex === index ? 'true' : 'false'">
            <a :href="p.id" class="result" :class="{
      selected: selectedIndex === index
    }" :aria-label="[...p.titles, p.title].join(' > ')" @mouseenter="!disableMouseOver && (selectedIndex = index)"
              @focusin="selectedIndex = index" @click="$emit('close')">
              <div>
                <div class="titles">
